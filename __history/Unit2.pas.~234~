...
begin
  if (ActiveControl is TDBGrid) then
    if not ExportToExcel((ActiveControl as TDBGrid), 'LOTOFACIL') then
      MessageBox(Handle, 'Não foi possível exportar para o excel', '', MB_OK + MB_ICONERROR);
end;
...
//      XApp.WorkBooks[1].WorkSheets.Add();
      ...
      ActiveWorkbook.PivotCaches.Create(1, "MAPA!R1C1:R10C10").CreatePivotTable "Dinamica!R3C1", "Tabela Dinâmica"
      ActiveWorkbook.ShowPivotTableFieldList = True
      ActiveSheet.PivotTables("Tabela dinâmica").ShowDrillIndicators = True
      ActiveSheet.PivotTables("Tabela dinâmica").DisplayFieldCaptions = True
      ActiveSheet.Cells(3, 1).Select
       }
//      sheet.ActiveWorkBook.PivotCaches.Create(1, sPlanName + '!R1C1:R' + IntToStr(row) + 'C' + IntToStr(dbGrid.FieldCount)).CreatePivotTable('Dinamica!R3C1', 'Tabela Dinâmica');
//      XApp.WorkBooks[1].WorkSheets['Dinamica'].PivotCaches.Create(1, sPlanName + '!R1C1:R' + IntToStr(row) + 'C' + IntToStr(dbGrid.FieldCount)).CreatePivotTable('Dinamica!R3C1', 'Tabela Dinâmica');
//      sheet.ShowPivotTableFieldList := True;
      try
        if bDinamicPlan then begin
          XApp.WorkBooks[1].PivotCaches.Create(1, sPlanName + '!R1C1:R' + IntToStr(row) + 'C' + IntToStr(Col)).CreatePivotTable('', 'Tabela Dinâmica');
    ...
    //      PivotCache.CreatePivotTable('Dinamica!R3C1', 'Tabela Dinâmica');
          sheet.PivotTables('Tabela dinâmica').ShowDrillIndicators := True;
          sheet.PivotTables('Tabela dinâmica').DisplayFieldCaptions := True;
